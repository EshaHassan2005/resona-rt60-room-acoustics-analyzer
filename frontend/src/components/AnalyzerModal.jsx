...
export default function AnalyzerModal({ isOpen, onClose, initialRoomType = 'recording_studio' }) {
  ...
  const [roomType, setRoomType] = useState(ROOM_TYPES_CONFIG[initialRoomType] ? initialRoomType : 'recording_studio');
  ...
  const generateSimulatedRoomModes = (L, W, H, rt60) => {
    ...
    const fftAmps = rawAmps.map(a => Number((0.12 + 0.76 * ((a - minA) / range)).toFixed(3)));
